...
exports.updateOrCreateSortInState = exports.ensureSortIdInState = exports.ensureSortsTypeInState = exports.ensureResourceTypeInState = exports.setIsInvalidatingForExistingResource = exports.updateOrInsertResourcesIntoState = exports.removeResourceFromState = exports.stateContainsResource = undefined;
...
var ensureSortsTypeInState = exports.ensureSortsTypeInState = function ensureSortsTypeInState(state, type) {
  var path = ['sorts', type];
  return (0, _utils.hasOwnProperties)(state, path) ? state : (0, _objectPathImmutable2.default)(state).set(path, {}).value();
};

var ensureSortIdInState = exports.ensureSortIdInState = function ensureSortIdInState(state, type, sortId) {
  var newState = ensureSortsTypeInState(state, type);
  var path = ['sorts', type, sortId];
  return (0, _utils.hasOwnProperties)(newState, path) ? newState : (0, _objectPathImmutable2.default)(newState).set(path, {}).value();
};
...
var updateOrCreateSortInState = exports.updateOrCreateSortInState = function updateOrCreateSortInState(state, payload) {
  ...
  var newState = ensureSortIdInState(state, type, sortId);
  var updatePath = ['sorts', type, sortId, 'ids'];
  var existingSort = (0, _utils.safeGet)(newState, updatePath, null);
  ...
  return (0, _objectPathImmutable2.default)(newState).set(updatePath, updatedSort).set(['sorts', type, sortId, 'total'], total).value();
};